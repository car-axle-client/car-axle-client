...
export default class Notification implements Component {
  ...
  constructor(parent: HTMLElement, title: string, body: string, button?: {text: string, fn: () => void}) {
    ...
    // custom stuff
    if (button) {
      createElement("button", this.notification, {className: "cac__notification__button", innerHTML: button.text})
        .onclick = e => button.fn()
    }
  }
  ...
  render() {
    ...
    createElement("button", this.notification, {className: "cac__notification__button", innerHTML: "Dismiss"})
